...
class StdImageField(ImageField):
    ...
    '''
    def __init__(self, verbose_name=None, name=None, width_field=None, height_field=None, size=None, cover_size=None, thumbnail_size=None, **kwargs):
        '''
        Added fields:
        ...
        for att_name, att in {'size': size, 'thumbnail_size': thumbnail_size, 'cover_size': cover_size}.items():
    ...
    def _get_cover_filename(self, filename):
        '''
        Returns the cover name associated to the standard image filename
            * Example: /var/www/myproject/media/img/picture_1.jpeg
                will return /var/www/myproject/media/img/picture_1.cover.jpeg
        '''
        splitted_filename = list(os.path.splitext(filename))
        splitted_filename.insert(1, '.cover')
        return ''.join(splitted_filename)
    ...
    def _rename_resize_image(self, instance=None, **kwargs):
        ...
        if getattr(instance, self.name):
            ...
            if os.path.abspath(filename) != os.path.abspath(dst_fullpath):
                ...
                if self.cover_size:
                    cover_filename = self._get_cover_filename(dst_fullpath)
                    shutil.copyfile(dst_fullpath, cover_filename)
                    self._resize_image(cover_filename, self.cover_size)
    ...
    def _set_cover(self, instance=None, **kwargs):
        '''
        Creates a "cover" object as attribute of the ImageField instance
        Cover attribute will be of the same class of original image, so
        "path", "url"... properties can be used
        '''
        if getattr(instance, self.name):
            filename = self.generate_filename(instance, os.path.basename(getattr(instance, self.name).path))
            cover_filename = self._get_cover_filename(filename)
            cover_field = ThumbnailField(cover_filename)
            setattr(getattr(instance, self.name), 'cover', cover_field)
    ...
    def delete_file(self, instance, sender, **kwargs):
        ''' This should be called when trying to delete the current file, from the database and the disk '''
        ...
        if file and file.name != self.default and \
            ...
            file.close()
        ImageField.delete_file(self, instance, sender, **kwargs)
    """
    ...
    def delete_thumbs(self, instance, sender, **kwargs):
        ''' This should be called when trying to delete the current file, from the database and the disk '''
        file = getattr(instance, self.attname)
        # If no other object of this type references the file,
        # and it's not the default value for future objects,
    ...
    def contribute_to_class(self, cls, name):
        ...
        signals.post_init.connect(self._set_cover, sender=cls)
        signals.pre_delete.connect(self.delete_thumbs,sender=cls)

#This code for South migration!
...
add_introspection_rules([
     (
        ...
        {           # Keyword argument
            ...
            "cover_size": ["cover_size", {"default": None}],
        },
    ),                    
], ["^std3image\.fields\.StdImageField"])